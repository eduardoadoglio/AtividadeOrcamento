...
<body>
 ...
 <?php
	...
	if( isset($_POST['nome_first']) && isset($_POST['nome_last']) && isset($_POST['senha']) && isset($_POST['confirm_senha']) && isset($_POST['email']) && isset($_POST['telefone']) && isset($_POST['permissao']) ){
		...
		if ($permissao == "Funcionario") {
			...
			if ($senha == $confirm_senha) {
				$sql = "insert into tb_funcionario (cd_funcionario, nm_first, nm_last, nm_senha, nm_email, nm_telefone, lv_permissao) VALUES (null,'$nome_first','$nome_last','$senha','$email','$telefone','$permissao')";
			}
			else{
		...
		if($mysqli->query($sql)){
				echo "<script>alert('Cadastro Efetuado com Sucesso - Admin TPO');window.location = 'index.php';</script>";
		}
		else 
		{
